...
constexpr auto VERSION = "0.7.2"; // MCPPPP version
...
namespace fsb
{
	...
	public:
		...
		inline nlohmann::json to_json() const
		{
			return
			{
				{ "sFactor", static_cast<int>(sFactor) },
				{ "dFactor", static_cast<int>(dFactor) },
				{ "equation", static_cast<int>(equation) }
			};
		}
	};

	inline const std::unordered_map<std::string, blend_t> blend =
	{
		{"add",
			{
				GL_SRC_ALPHA,
		...
		{"dodge", // TODO: this is currently linear dodge, same as add (somewhat inaccurate, approximation at best)
			{
				GL_SRC_ALPHA,
				GL_ONE,
				GL_FUNC_ADD
		...
		{"overlay", // TODO: currently screen, should be multiply if base layer is dark (needs official fsb support)
			{
				GL_ONE,
				GL_ONE_MINUS_SRC_COLOR,
				GL_FUNC_ADD
